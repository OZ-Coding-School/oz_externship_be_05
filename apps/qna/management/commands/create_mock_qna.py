...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        try:
            ...
            answerer_1 = User.objects.get(id=6)
            ...
            a1 = Answer.objects.create(
                question=q,
                author=answerer_1,
                content="6번 유저가 작성한 첫 번째 테스트 답변입니다."
            )
            ...
            a2 = Answer.objects.create(
                question=q,
                author=answerer_2,
                content="7번 유저가 작성한 두 번째 테스트 답변입니다."
            )
            ...
            AnswerComment.objects.create(
                answer=a1,
                author=questioner,
                content="질문자(4번)가 6번 답변에 단 댓글입니다."
            )

            self.stdout.write(self.style.SUCCESS(
                f"성공적으로 데이터를 생성했습니다.\n"
                f"- 질문(ID:{q.id}): 작성자 4번({questioner.nickname})\n"
                f"- 답변 1: 작성자 6번({answerer_1.nickname})\n"
