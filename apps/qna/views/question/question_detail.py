...
from apps.qna.serializers.question.question_ai_answer_serializer import (
    QuestionAIAnswerSerializer,
)
...
class QuestionAIAnswerAPIView(APIView):
    ...
    @extend_schema(
        tags=["질의응답"],
        summary="AI 답변 생성",
        description="질문 작성 후, 해당 질문에 대해 GET 요청 시 AI 최초 답변을 생성/저장하고 반환합니다.\n"
        ...
        responses={
            ...
            403: OpenApiResponse(
                EMS.E403_PERMISSION_DENIED("AI 답변 생성"),
                description="권한 없음",
                examples=[
                    OpenApiExample(
                        name="권한 없음",
                        summary="해당 유저가 작성한 질문이 아니면 AI답변이 생성되지 않음.",
                        value={"error_detail": "AI 답변 생성 권한이 없습니다."},
                    )
                ],
            ),
            404: OpenApiResponse(
                EMS.E404_NOT_EXIST("질문"),
                description="질문을 찾을 수 없음",
                examples=[
                    OpenApiExample(
                        name="질문을 찾을 수 없음",
                        summary="AI답변을 생성하기 위한 질문Question이 존재하지 않음.",
                        value={"error_detail": "등록된 질문이(가) 없습니다."},
                    )
                ],
            ),
            409: OpenApiResponse(
                EMS.E409_AI_ALREADY_RESPONDED,
