...
class BaseMixin:
    # [이메일] [email]
    @staticmethod
    def get_email(**kwargs: Any) -> serializers.EmailField:
        """[🌸] validate가 없습니다"""
        return serializers.EmailField(**_merge_defaults({"required": True}, kwargs))

    # [인증코드] [verify_code] [code]
    @staticmethod
    def get_verify_code(**kwargs: Any) -> serializers.CharField:
        """[☠️] validate가 있습니다 !"""
        code_length = getattr(settings, "VERIFYCATION_CODE_LENGTH", 6)
