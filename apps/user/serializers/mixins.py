...
class SMSTokenMixin(BaseMixin):
    sms_token = BaseMixin.get_verify_token_field(write_only=True)
...
class EmailTokenMixin(BaseMixin):
    email_token = BaseMixin.get_verify_token_field(write_only=True)

    def validate_email_token(self, value: str) -> str:
        return BaseMixin.validate_verify_token(self, value)


class AllTokensMixin(SMSTokenMixin, EmailTokenMixin):
    pass


class SenderTokenMixin:
