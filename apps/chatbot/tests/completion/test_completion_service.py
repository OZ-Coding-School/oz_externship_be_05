...
class CompletionResponseServiceTests(APITestCase):
    ...
    def test_sse_encode(self) -> None:
        self.assertEqual(svc._sse_encode("hello"), "data: hello\n\n")
        self.assertEqual(svc._sse_encode(""), "data: \n\n")

    def test_sse_json(self) -> None:
        # 일반 content
        self.assertEqual(svc._sse_json("테스트"), 'data: {"content": "테스트"}\n\n')
        # done / error
        self.assertEqual(svc._sse_json("", done=True), "data: [DONE]\n\n")
        self.assertEqual(svc._sse_json("", error=True), "data: [ERROR]\n\n")
        # done 우선
        self.assertEqual(svc._sse_json("x", done=True, error=True), "data: [DONE]\n\n")
        # ensure_ascii=False 기대
        self.assertNotIn("\\u", svc._sse_json("한글"))

    # DB 저장 확인
    def test_user_message_save(self) -> None:
        c = svc.user_message_save(session=self.session, message="hi")
    ...
    def test_ai_message_save(self) -> None:
        c = svc.ai_message_save(session=self.session, message="hello")
        self.assertEqual(c.session_id, self.session.id)
    ...
    def test_build_contents_appends_user_message(self) -> None:
        ...
        contents = svc._build_contents(session=self.session, user_message="새 메시지")
    ...
    @patch("apps.chatbot.services.completion_response_service._get_client")
    def test_iter_gemini_text_stream_yields_only_text(self, mock_get_client: MagicMock) -> None:
        ...
        out = list(svc._iter_gemini_text_stream(contents=contents))
    ...
    def test_generate_streaming_response_success_saves_full_message(self, mock_iter: MagicMock) -> None:
        ...
        self.assertEqual(out[0], svc._sse_json("Hello "))
        self.assertEqual(out[1], svc._sse_json("World"))
    ...
    def test_create_streaming_response_headers(self) -> None:
        resp = svc.create_streaming_response(session=self.session, user_message="테스트")
